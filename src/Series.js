...
class Series extends Component {
	...
	loadData() {
		...
		if (this.props.match.params.genre) {
			api.loadSeriesByGenre(this.props.match.params.genre).then((res) => this.setState({
				isLoading: false,
				series: res.data
			}))
		}else{
			api.loadSeries().then((res) => this.setState({
				isLoading: false,
				series: res.data
			}))
		}
		...
		return (
			<section id="intro" className="intro-section">
				...
				{!this.state.isLoading && this.state.series.length === 0 &&
					<div className='alert alert-info'> Nenhuma série no momento. </div>
				}
